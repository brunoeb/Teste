...
begin
  ...
  {$REGION 'INICIO VALIDACOES'}
    ...
    if DT_PREVISAO < DT_CADASTRO then begin
       Result := false;
       erro := 'A data de previsão de entrega não pode ser menor que a data de cadastro ';
       exit;
    end;
   ...
   try
     ...
     with qry do begin
       ...
       SQL.Add('SELECT COUNT(IDTAREFA) QTDE, PRIORIDADE,'+
               'IIF((select COUNT(IDTAREFA) from [DBO].TAREFAS where (STATUSTAREFA <> '+QuotedStr('Encerrada')+')) >0, '+
               'CAST((COUNT(IDTAREFA)/'+
               'cast((select COUNT(IDTAREFA) from [DBO].TAREFAS where (STATUSTAREFA <> '+QuotedStr('Encerrada')+'))as numeric(18,2)))*100 as numeric(9,2)),0) MEDIA '+
               'FROM [DBO].TAREFAS '+
               'WHERE (STATUSTAREFA <> '+QuotedStr('Encerrada')+') '+
               'GROUP BY PRIORIDADE '+
               'ORDER BY 3 DESC   ');
       Active := true;
     end;
