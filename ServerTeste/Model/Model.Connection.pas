   ...
   try
      try
         ...
         if not FileExists(arq_ini) then begin
            Result := 'Arquivo INI não enontrado ' + arq_ini;
            Exit;
         end;
         ...
         ZConn.Database    := ini.ReadString('Banco de Dados', 'Database', '');//'Provider=MSDASQL.1;Data Source=tarefas';
         ZConn.LoginPrompt := false;
         ZConn.HostName    := ini.ReadString('Banco de Dados', 'HostName', '');//'BRUNO\SQLEXPRESS';
         ZConn.Protocol    := ini.ReadString('Banco de Dados', 'Protocol', '');//'ado';
